...
def student_edit(request,pk):
    ...
    if request.method =='POST':
        ...
        curr_branch = models.Branch.objects.get(name=request.session['curr_branch'])
        student = models.RsquareUser.objects.get(pk = pk)
        profile = models.StudentProfile.objects.get(student=student)
        student.first_name=request.POST['first_name']
        student.last_name=request.POST['last_name']
        student.email=request.POST['email']
        student.role='STUDENT'
        student.branch_id=curr_branch.id
        ...
        profile.aadhar_num=request.POST['aadhar']
        ...
        args = {'student':student,'profile':profile,'CATEGORY_CHOICES':CATEGORY_CHOICES,'GENDER_CHOICES':GENDER_CHOICES,'dob':dob}
        return render(request, 'student/student_edit.html',args)
...
def student_guardian(request, pk):
    ...
    if not (request.user.is_owner() or request.user.is_branch_manager()):
        return redirect('dashboard')
    if request.method == 'POST':
        guardian = models.Guardian.objects.create(relation=request.POST['relation'], salutation=request.POST['salutation'],
        ...
        guardiangroup = models.GuardianGroup.objects.create(student_id = student.profile.pk,guardian = guardian)
        return redirect('student-guardian-add', pk=student.id)
    else:
        guardians = models.GuardianGroup.objects.select_related().filter(student_id = student.profile.pk)
        args ={'guardians':guardians,'pk':student.pk,'student':student}
...
def student_courses(request, pk):
    ...
    if request.method == 'POST':
        ...
        courses = models.Course.objects.filter(branch__name=request.session['curr_branch'])
        return render(request, 'student/course_form.html', {'courses': courses,'pk':pk})
...
def student_installments(request, pk):
    ...
    if request.method == 'POST':
        ...
        return render(request, 'student/installments_form.html',{'pk':pk})
